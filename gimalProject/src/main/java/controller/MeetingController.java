...
public class MeetingController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		case "/list": {
			...
			String dateFrom = req.getParameter("dateFrom");   // 시작일 (yyyy-MM-dd)
			String dateTo   = req.getParameter("dateTo");     // 종료일 (yyyy-MM-dd)
			...
			if (hasFilter) {
				// 기간 + 카테고리 + 키워드 + 상태 + 날씨 필터 적용
				aList = meetingService.getMeetingListFiltered(
						category, dateFrom, dateTo, keyword, status, weather);
			} else {
				// 필터 없을 때 전체 리스트
				aList = meetingService.getMeetingList();
			}
		...
		case "/info": {
			...
			if (userId != -1) {
				isParticipant = meetingService.isParticipant(
						Long.parseLong(req.getParameter("meetingId")), userId);
			}

			req.setAttribute("isParticipant", isParticipant);

			MeetingInfoDTO infoDto = meetingService.getMeetingInfo(
					Long.parseLong(req.getParameter("meetingId")));
